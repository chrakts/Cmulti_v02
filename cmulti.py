...
class CMULTI(object):
  def __init__(self,source,comPort="", baudRate=57600, backChannel="Klima", withCrc = True, timeout=1000):
    if comPort.lower() != "mqtt":
      ...
      path = "/home/chrak/Daten/Software/repositories/bridges"
  ...
  def sendStandard_mqtt(self,text,target,function,address,job,dataType,expectAnswer = True):
    ...
    if expectAnswer == True:
      msg = subscribe.simple("Answer/%s/#"%(target), qos=0, msg_count=1, hostname=self.dataMap["mqtt"]["serverIP"],port=1883, keepalive=60, auth=self.auth)
